...
public class GUIVisualization extends JFrame {
  ...
  private List<Integer> dataPointsY; // List to store y-axis data points
  ...
  public void setDataPointsX(List<Integer> dataPointsX) {
    this.dataPointsX = dataPointsX;
  }

  public void setDataPointsY(List<Integer> dataPointsY) {
  ...
  private void drawGraph(Graphics g) {
    ...
    long maxYValue = getMaxYValue(); // Get maximum Y value
    ...
    for (int i = 0; i < numberYDivisions + 1; i++) {
      ...
      if (dataPointsY.size() > 0) {
        ...
        String yLabel =
          ((int) ((maxYValue * ((i * 1.0) / numberYDivisions)) * 100)) /
          100.0 +
    ...
    for (int i = 0; i < dataPointsX.size(); i++) {
      if (dataPointsX.size() > 1) {
        ...
        if ((i % ((int) ((dataPointsX.size() / 20.0)) + 1)) == 0) {
          ...
          g2.drawLine(
            x0,
            height - padding - labelPadding - 1 - labelPadding,
            x1,
            padding
          ); // Draw grid line
          g2.setColor(Color.BLACK); // Set color for labels
  ...
  private long getMaxYValue() {
    ...
    for (Integer y : dataPointsY) {
      max = Math.max(max, y); // Find maximum y value
    }
    return max == Long.MIN_VALUE ? 1 : max; // Prevent division by zero
  }
